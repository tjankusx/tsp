...
function getCentroid($fromNodes) {

	$centroidX = 0;
	$centroidY = 0;
	$det = 0;
	$nNodes = count($fromNodes);
	...
	for ($i = 0; $i<$nNodes; $i++)
	{
		...
		$tempDet = $fromNodes[$i]->x * $fromNodes[$j]->y - $fromNodes[$j]->x * $fromNodes[$i]->y;
		$det += $tempDet;

		$centroidX += ($fromNodes[$i]->x + $fromNodes[$j]->x)*$tempDet;
		$centroidY += ($fromNodes[$i]->y + $fromNodes[$j]->y)*$tempDet;
	}
...
$lastNodeInserted = addNode($toImport[5][0]*8, $toImport[5][1]*8);
unset($toImport[5]);
...
for ($counter=0; $counter<20; $counter++) {
	...
	echo count($toImport) . 'nodes left...' . PHP_EOL;
    }
...
// let's just do furthest from centroid
    ...
    foreach ($toImport as $i => $data) {
        ...
        $dist = sqrt(pow($data[0]*8 - $centroid[0], 2) + pow($data[1]*8 - $centroid[1], 2));
...
$col_ellipse_noninc = imagecolorallocate($image, 64, 64, 128);
$col_ellipse_centroid = imagecolorallocate($image, 255, 32, 32);
...
foreach ($nodes as $node) {
    ...
    $nodePoints[] = $node->x;
    $nodePoints[] = $node->y;
    imagefilledellipse($image, $node->x, $node->y, 2, 2, $col_ellipse);
}

foreach ($toImport as $node) {
    imagefilledellipse($image, $node[0]*8, $node[1]*8, 2, 2, $col_ellipse_noninc);
}

imagefilledellipse($image, $centroid[0], $centroid[1], 2, 2, $col_ellipse_centroid);
...
file_put_contents('1500nodes_distance.txt', 'Total: ' . $totalDistance/8 .
	PHP_EOL . 'Nodes included: ' . count($nodes) .
	PHP_EOL . 'Nodes available: ' . count($toImport));



if (php_sapi_name() == 'cli') {
    echo 'Total: ' . $totalDistance/8 .
	PHP_EOL . 'Nodes included: ' . count($nodes) .
	PHP_EOL . 'Nodes available: ' . count($toImport);
    return;
}
